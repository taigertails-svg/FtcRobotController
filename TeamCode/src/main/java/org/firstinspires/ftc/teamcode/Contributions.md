...
1. No vibe coding. (no AI)
2. Variables should preferably not be shortened. (private Servo SL would be wrong.)
3. All variables should be pascal case. (for example: int HelloWorld)
4. If you added a variable and you're not planning on changing it then add the final keyword before it.
5. All constants should be screaming snake case. (for example: final int HELLO_WORLD)
6. All code should compile without any errors.
7. Code should preferably not have any warnings. (although there are some exceptions.)
8. Comments should have proper grammar.
9. If you're using an if statement to only do a one line statement then use a shorthand if.
10. When adding a new method to a class make sure to add tests to it.
11. Don't add unnecessary space in the code unless you're sure that it's going to become more readable.
12. Make all of the variables you're gonna use in your method at the start of it.
13. Don't use magic numbers, instead add it to constants and use the constant instead. (https://en.wikipedia.org/wiki/Magic_number_(programming))
